...
export default function ProductCard({ product, isSeller = false, isClickable = true, onClick }: Props) {
  const showAsButton = isSeller && isClickable;
  const Comp = showAsButton ? 'button' : 'div';
  const compProps = showAsButton
    ...
    : { className: 'flex flex-col border border-gray-200 rounded-lg overflow-hidden bg-white hover:border-primary/30 transition-colors' };
      ...
      <div className="relative w-full bg-gray-50 h-[180px] max-h-[200px] md:h-[220px] md:max-h-none lg:h-[260px]">
        ...
        {isSeller && (
          <div className="absolute top-2 left-2 flex gap-1.5 z-10">
            ...
            <Link
              to={`/seller/offers/add/${product.id}`}
              className="bg-white/90 hover:bg-white text-gray-700 text-[10px] font-medium px-2 py-1 rounded-full shadow transition-colors"
            >
              + عرض
            </Link>
